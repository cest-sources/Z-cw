...
% setup pool system parameters
    ...
    % CEST pool 'b'
    P.fB=0.0018018;     % proton fraction e.g. 50mM creatine in water:  4*50/(2*55.5*1000)=0.0018018;
    P.kBA=5;          % exchange rate [s^-1]     % corresponds to creatine at ~ 22°C, pH=6.4 in PBS (Goerke et al.)
...
%     P.fC=0.9;                 % proton fraction of the semi-solid pool
%     P.kCA=40;                 % exchange rate [s^-1]
%     P.dwC=-2;                 % deltaW_B in [ppm} (chemical shift)
%     P.R2C=1/(8*10^-6);        % transversal relaxation rate 1/T2 of pool c [s^-1]
...
% setup sequence parameters
    ...
    P.B1=3;                 % irradiation amplitude [µT]
    P.tp=5;                % pulse duration = saturation time [s]
    P.xZspec= [-3:0.1:3];   % chemical shift of the CEST pool in [ppm] 
